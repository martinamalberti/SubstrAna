#include "../include/QGLikelihoodCalculator.h"
...
QGLikelihoodCalculator::QGLikelihoodCalculator( const std::string dataDir, Bool_t chs){
  std::string histoFileName = "ReducedHisto_2012.root";
  if(chs) histoFileName = "ReducedHisto_2012_CHS.root";
  histoFile_ = TFile::Open((edm::FileInPath(dataDir+histoFileName).fullPath()).c_str());
  nPtBins_ = 21;
  nRhoBins_ = 45 ;
...
Float_t QGLikelihoodCalculator::QGvalue(std::map<std::string, Float_t> variables){
