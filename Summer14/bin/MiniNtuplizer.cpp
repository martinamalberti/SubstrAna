...
double correctionNoL1(PseudoJet &iJet,FactorizedJetCorrector *iJetCorr,double iRho){
  ...
  cout << "full corr = " << jetcorr << endl;
  cout << "L2L3only  = " << jetcorr/lCorrections[0] <<endl;  
...
int main (int argc, char ** argv) {
  ...
  */
  ...
  for(int ientry = 0; ientry < maxEvents; ientry++) { 
    ...
    fillRecoJetsInfo(softJets, jet_def_Pruning, soft_event  , JSoftKillerInfo  , JGenInfo, true , jetCorr, jetUnc, applyOnlyL2L3JEC, gsn_cleanser,nPU);
