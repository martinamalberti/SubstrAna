...
def writeJobs(wdir, indir, outputname, ptmin, ptmax, etamin, etamax, njobs):
    #---------------------------------------------
    ...
    for jobid in range(njobs):         
        ...
        jobscript.write('\t TestJetTreeAnalysis %s/input_%d.txt %s_%d.root %f %f %f %f 0'%(jobdir, jobid, outputname, jobid, ptmin, ptmax, etamin, etamax))
...
if not options.checkJobs and not options.resubmit:
    ...
    writeJobs(workingdir, options.inputdir, options.outputname, options.ptmin, options.ptmax, options.etamin, options.etamax, options.njobs )
